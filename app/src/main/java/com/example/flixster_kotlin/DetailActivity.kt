...
private const val YOUTUBE_API_KEY="AIzaSyDAJAfWbgoLWsONfZ2plqChskEtrZqMkq4"
...
class DetailActivity : YouTubeBaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.i(TAG, "Moive is $movie")
        tvTitle.text = movie.title
        tvOverView.text= movie.overView
        //GetTODO: the rating of the movie
        ...
        client.get(TRAILERS_URL.format(movie.movieId), object: JsonHttpResponseHandler(){
            ...
            override fun onSuccess(statusCode: Int, headers: Headers?, json: JSON ) {
                ...
                if (results.length() == 0){
                    Log.w(TAG, "no movie trailter found")
                    return
                }
                val movieTrailerJson = results.getJSONObject(0)
                val youtubeKey = movieTrailerJson.getString("key")
                //play the youtube vidoe with this trailer
